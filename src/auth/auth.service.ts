...
export class AuthService {
  ...
  async validateSocialLogin(
    ...
    if (user) {
      ...
      user = await this.usersService.create({
        ...
        status,
        /**
         * The country is not required, but if it is, then you need to add it to the user entity
         * country: CountryEnum;
         */
        country: CountryEnum.SENEGAL,

        // a user must be added to an ethic committee
        // the ethic is from the database
        ethicCommittee: plainToClass(EthicCommittee, {
          id: 1,
        }),
      });
  ...
  async register(dto: AuthRegisterLoginDto): Promise<void> {
    ...
    const user = await this.usersService.create({
      ...
      country: CountryEnum.SENEGAL,
      // ethicCOmmittee
      ethicCommittee: EthicCommittee[CountryEnum.SENEGAL],
    });
